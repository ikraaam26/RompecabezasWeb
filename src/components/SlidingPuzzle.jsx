...
export default function Puzzle({ imageUrl, nombre, idImagen }) {
  ...
  // Nuevo estado para el botón de inicio
  const [gameStarted, setGameStarted] = useState(false);

  // Iniciar el temporizador cuando se inicia el juego
  useEffect(() => {
    let interval;
  ...
  const loadPuzzle = async () => {
    ...
    image.onload = () => {
      ...
      setIsLoading(false);
      // Ya no activamos el temporizador aquí, esperamos al botón de inicio
    };
    image.src = imageUrl;
  };
  ...
  const handleClick = (i) => {
    // Verificar si el juego ha comenzado antes de permitir movimientos
    if (puzzleComplete || isLoading || !gameStarted) return;
  ...
  const saveGameData = async (finalPoints, finalCoins) => {
    try {
      ...
      // 2. Primero obtenemos los datos actuales del usuario para calcular el nuevo total
      const { data: userData, error: fetchError } = await supabase
        .from('usuarios')
        .select('totalpuntos, totalmonedas')
        .eq('id', user.id)
        .single();
      ...
      // 3. Calculamos los nuevos totales
      const newTotalPuntos = (userData.totalpuntos || 0) + finalPoints;
      const newTotalMonedas = (userData.totalmonedas || 0) + finalCoins;
     
      // 4. Actualizamos el usuario con los nuevos totales
      const { error: updateError } = await supabase
        .from('usuarios')
        .update({
          totalpuntos: newTotalPuntos,
          totalmonedas: newTotalMonedas
        })
        .eq('id', user.id);
      ...
      // 5. Guardar la partida en la tabla Partidas
        ...
        <div className="flex flex-col gap-4 p-4 bg-gray-800 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              disabled={gameStarted} // Deshabilitar cambio de dificultad cuando el juego ha iniciado
            >
          ...
          {/* Botón de iniciar */}
          {!gameStarted && !puzzleComplete && !isLoading ? (
            <button
              onClick={startGame}
        ...
        <div className="flex flex-col items-center">
          {isLoading ? (
            ...
            <div className="relative">
              {/* Overlay de "Pulse Comenzar" si el juego no ha iniciado */}
              {!gameStarted && !puzzleComplete && (
                <div className="absolute inset-0 bg-black/70 flex items-center justify-center z-10 rounded-md">
                  <div className="text-white text-xl font-bold text-center">
                    <p>Pulse "Comenzar Juego"</p>
                    <p className="text-sm mt-2 text-cyan-400">para iniciar el rompecabezas</p>
                  </div>
                </div>
              )}
